...
if uploaded_file is not None:
    new_faq = pd.read_csv(uploaded_file, encoding="utf-8")
...
if do_index or (st.session_state.vectorizer is None):
    corpus = (st.session_state.faq_df["question"].astype(str)+" "+st.session_state.faq_df["answer"].astype(str)).tolist() # if st.session_state.vectorizer else []
